...
public class LinkedTeleportControllerItemRenderer extends CustomRenderedItemModelRenderer<LinkedTeleportControllerModel> {
	...
	static void tick() {
		...
		boolean active = LinkedTeleportControllerClientHandler.MODE != LinkedTeleportControllerClientHandler.Mode.IDLE;
		...
		for (int i = 0; i < buttons.size(); i++) {
			...
			lerpedFloat.chase(LinkedTeleportControllerClientHandler.currentlyPressed.contains(i) ? 1 : 0, .4f, Chaser.EXP);
	...
	@Override
	protected void render(ItemStack stack, LinkedTeleportControllerModel model, PartialItemModelRenderer renderer,
	...
	protected static void renderNormal(ItemStack stack, LinkedTeleportControllerModel model,
	...
	public static void renderInLectern(ItemStack stack, LinkedTeleportControllerModel model,
	  	PartialItemModelRenderer renderer, TransformType transformType, PoseStack ms,
	...
	protected static void render(ItemStack stack, LinkedTeleportControllerModel model,
	  	PartialItemModelRenderer renderer, TransformType transformType, PoseStack ms,
		...
		if (renderType == RenderType.NORMAL) {
			...
			active = false;
			boolean noControllerInMain = !ModItems.LINKED_TELEPORT_CONTROLLER.isIn(mc.player.getMainHandItem());
			...
			active &= LinkedTeleportControllerClientHandler.MODE != LinkedTeleportControllerClientHandler.Mode.IDLE;
		...
		if (renderType == RenderType.NORMAL) {
			if (LinkedTeleportControllerClientHandler.MODE == LinkedTeleportControllerClientHandler.Mode.BIND) {
	...
	@Override
	public LinkedTeleportControllerModel createModel(BakedModel originalModel) {
		return new LinkedTeleportControllerModel(originalModel);
	}
