...
public class LinkedTeleportControllerClientHandler {

	public static final IGuiOverlay OVERLAY = LinkedTeleportControllerClientHandler::renderOverlay;
	...
	protected static void onReset() {
		...
		if (!currentlyPressed.isEmpty())
			ModPackets.channel.sendToServer(new LinkedTeleportControllerInputPacket(currentlyPressed, false));
		currentlyPressed.clear();

		LinkedTeleportControllerItemRenderer.resetButtons();
	}

	public static void tick() {
		LinkedTeleportControllerItemRenderer.tick();
		...
		if (MODE == Mode.ACTIVE) {
			...
			if (!releasedKeys.isEmpty()) {
				ModPackets.channel.sendToServer(new LinkedTeleportControllerInputPacket(releasedKeys, false, lecternPos));
				AllSoundEvents.CONTROLLER_CLICK.playAt(player.level, player.blockPosition(), 1f, .5f, true);
			}

			// Newly Pressed Keys
			if (!newKeys.isEmpty()) {
				ModPackets.channel.sendToServer(new LinkedTeleportControllerInputPacket(newKeys, true, lecternPos));
			...
			if (packetCooldown == 0) {
				if (!pressedKeys.isEmpty()) {
					ModPackets.channel.sendToServer(new LinkedTeleportControllerInputPacket(pressedKeys, true, lecternPos));
					packetCooldown = PACKET_RATE;
				}
			}
		}
		...
		if (MODE == Mode.BIND) {
			...
			for (Integer integer : newKeys) {
				...
				if (linkBehaviour != null) {
					ModPackets.channel.sendToServer(new LinkedTeleportControllerBindPacket(integer, selectedLocation));
					Lang.translate("simple_teleport_controller.key_bound", controls.get(integer)
	...
	public static void renderOverlay(ForgeGui gui, PoseStack poseStack, float partialTicks, int width1,
		...
		list.add(Lang.translateDirect("simple_teleport_controller.bind_mode")
